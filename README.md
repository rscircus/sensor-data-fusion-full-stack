...
The example above is generated by simply predicting (adding) and (Bayesian) correcting (multiplying) Gaussians. See how the filter changes it's assumption upon the heavy measurement offset around frame 15. The evolution model is assuming the start location at `0` and the velocity to be `1` with error `0.05`. Watch also, how the filter corrects the variance quite fast. The init is insanely huge `10^5`.
