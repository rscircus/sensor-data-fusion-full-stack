...
# This simulation contains a "negative information' measurement. 
# That is, the measurements Z^k are missing one measurement (detected
# with a 0) and watch the pdf change.
...
def animate(frame):
    ...
    if z[0] == 0.0:
        ...
        R = np.array([[np.power(10,6), 0], [0.0, np.power(10,6)]])  # measurement error covariance matrix
        S = H @ P @ H.T + R
